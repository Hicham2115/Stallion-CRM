...
export function ConsoleTopbar() {
    ...
    return (<header className="sticky top-0 z-30 flex min-h-[4.5rem] shrink-0 items-center gap-4 border-b border-hairline bg-deck-bar/90 px-4 py-3 backdrop-blur-xl sm:px-6">
      <span data-print="hide">
        <MobileNav role={user.role}/>
      </span>

      <div className="min-w-0 flex-1">
        {override.parent && (<Link href={override.parent.href} className="-ml-1 mb-0.5 inline-flex items-center gap-1 rounded text-[0.75rem] text-ink-muted transition-colors hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/60">
        ...
        {subtitle && (<p className="mt-0.5 hidden truncate text-[0.8125rem] text-ink-muted sm:block">
            {subtitle}
          </p>)}
      </div>

      <div data-print="hide" className="flex shrink-0 items-center gap-2 sm:gap-3">
        <MockDataChip />

        {consoleConfig.features.notifications && (<button type="button" aria-label={consoleConfig.content.notificationsLabel} className="grid size-10 place-items-center rounded-xl border border-hairline bg-white/[0.03] text-ink-soft transition-colors hover:bg-white/[0.06] hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/60">
