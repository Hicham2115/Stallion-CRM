...
export function ThreadList({ reps, threads, selectedThreadId, onSelect, }) {
    return (<ul className="deck-scroll flex-1 overflow-y-auto p-2">
      {reps.map((rep) => {
            ...
            <button type="button" onClick={() => onSelect(thread.id)} aria-current={selected ? "true" : undefined} className={cn("flex w-full items-center gap-3 rounded-md px-2.5 py-2.5 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/60", selected
