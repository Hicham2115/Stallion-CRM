...
// Submits on Ctrl/Cmd+Enter as well as the button — plain Enter inserts a
// newline since a note is multi-line, but the modifier still gives a
// keyboard path.
export function NotesPanel({ leadId, notes, }) {
    ...
    return (<Panel className="flex h-full flex-col">
      ...
      <PanelBody className="flex flex-1 flex-col">
        {features.notes && (<form onSubmit={handleSubmit} className="mb-5" data-print="hide">
            <label htmlFor="lead-note" className="sr-only">
              {content.addNoteLabel}
            </label>
            <textarea id="lead-note" rows={2} value={body} onChange={(event) => setBody(event.target.value)} onKeyDown={(event) => {
                if ((event.metaKey || event.ctrlKey) && event.key === "Enter") {
                    void handleSubmit(event);
                }
            }} placeholder={content.addNotePlaceholder} className="deck-input w-full resize-none rounded-xl border border-hairline bg-white/[0.02] px-3.5 py-3 text-[0.875rem] leading-relaxed text-ink caret-brand outline-none transition duration-200 placeholder:text-ink-muted hover:border-hairline-strong hover:bg-white/[0.035] focus:border-brand/55 focus:bg-white/[0.05] focus:ring-4 focus:ring-brand/10"/>

            <div className="mt-2 flex justify-end">
              <Button type="submit" variant="outline" size="sm" disabled={!canSubmit}>
                {pending ? (<LoaderCircle aria-hidden className="deck-spin size-3.5"/>) : null}
                {content.addNoteSubmit}
              </Button>
            </div>
          </form>)}

        {notes.length === 0 ? (<EmptyState icon={StickyNote} title={content.notesEmpty}/>) : (<ul className="flex flex-col gap-3">
            {notes.map((note) => (<li key={note.id} className="rounded-xl border border-hairline bg-white/[0.02] p-3.5">
