...
// Phone and email are actionable links, not printed text.
export function ContactPanel({ lead }) {
    const rows = [
        {
            icon: Phone,
    ...
    return (<Panel className="flex h-full flex-col">
      ...
      <PanelBody className="flex flex-1 flex-col">
        <ul className="flex flex-col gap-3.5">
          {rows.map((row) => {
            const Icon = row.icon;
            return (<li key={row.value} className="flex items-center gap-3">
                ...
                {row.href ? (<a href={row.href} className={`min-w-0 flex-1 truncate rounded text-[0.875rem] text-ink-soft transition-colors hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/60 ${row.mono ? "deck-nums" : ""}`}>
                    {row.value}
                  </a>) : (<span className="min-w-0 flex-1 truncate text-[0.875rem] text-ink-soft">
                    {row.value}
                  </span>)}
              </li>);
        })}
