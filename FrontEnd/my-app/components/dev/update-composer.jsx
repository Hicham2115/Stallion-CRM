...
// Writes lead.updates — the client-facing "Latest updates" feed. Distinct from
// lead.activity (internal sales history the client never sees); see the
// CLIENT-SAFE RULE in config/portal.ts.
export function UpdateComposer({ lead }) {
      ...
      <PanelBody className="flex flex-1 flex-col">
        ...
        <form onSubmit={handleSubmit} noValidate className="mt-4 space-y-3">
          <div>
            ...
            <input id="update-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder={copy.titlePlaceholder} aria-invalid={Boolean(error)} aria-describedby={error ? "update-error" : undefined} className={cn(fieldBase, "mt-2 h-11 w-full px-3.5")}/>
          </div>
          ...
          <div>
            ...
            <textarea id="update-body" rows={3} value={body} onChange={(event) => setBody(event.target.value)} placeholder={copy.bodyPlaceholder} className={cn(fieldBase, "mt-2 w-full resize-y px-3.5 py-2.5 leading-relaxed")}/>
          </div>

          <div aria-live="polite">
            {error && (<p id="update-error" role="alert" className={fieldErrorText}>
                {error}
              </p>)}
          </div>
    ...
    disabled={pending || title.trim().length === 0} className="h-11 font-semibold">
              {pending ? (<LoaderCircle aria-hidden className="deck-spin size-4"/>) : (<Send aria-hidden className="size-4"/>)}
        ...
        <div className="mt-auto border-t border-hairline pt-4 mt-5">
          ...
          {recent.length === 0 ? (<p className="mt-2 text-[0.8125rem] text-ink-muted">
              {copy.emptyDescription}
            </p>) : (<ul className="mt-3 flex flex-col gap-2.5">
              {recent.map((update) => (<li key={update.id} className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
